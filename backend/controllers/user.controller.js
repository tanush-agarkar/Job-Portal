...
export const register = async (req, res) => {
  try {
    const { fullname, registrationNo, email, phoneNumber, password, role } =
      req.body;
    ...
    // Validate email format: registrationNo@ycce.in
    const expectedEmail = `${registrationNo}@ycce.in`;
    if (email !== expectedEmail) {
      return res.status(400).json({
        message: `Email must be in the format ${registrationNo}@ycce.in.`,
        success: false,
      });
    }
...
export const updateProfile = async (req, res) => {
  try {
    const { fullname, registrationNo, email, phoneNumber, bio, skills } =
      req.body;
